...
def get_official_eval_result(gt_annos,
    ...
    detail = {}
    best_ap = 0.0
    best_log = collections.defaultdict(float)
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            if wandb_log and min_overlaps[i, 0, j] > .5 and min_overlaps[i, 1, j] > .5 and min_overlaps[i, 2, j] > .5:
                for k, v in enumerate(mAPbbox.split(',')):
                    wandb.log({f"{class_to_name[curcls]}_bbox.7_{k}": float(v)}) 
                    best_log[f"{class_to_name[curcls]}_bbox.7_{k}"] = float(v)
                for k, v in enumerate(mAPbev.split(',')):
                    wandb.log({f"{class_to_name[curcls]}_bev.7_{k}": float(v)})
                    best_log[f"{class_to_name[curcls]}_bev.7_{k}"] = float(v)
                    best_ap += float(v)
                for k, v in enumerate(mAP3d.split(',')):
                    wandb.log({f"{class_to_name[curcls]}_3d.7_{k}": float(v)})
                    best_log[f"{class_to_name[curcls]}_3d.7_{k}"] = float(v)
                    best_ap += float(v)
    return {
        "result": result,
        "detail": detail,
...
def eval_from_scrach(gt_dir, det_dir, eval_cls_list=None, ap_mode=40, logger=None, wandb_log=False):
    global AP_mode
    AP_mode = ap_mode
    ...
    for cls in eval_cls_list:
        ...
        res = get_official_eval_result(all_gt, all_det, cls, z_axis=1, z_center=1, wandb_log=wandb_log)
        Car_res = res['detail'][cls]
        best_ap += res['best_ap']
        best_log = {**best_log, **res['best_log']}
        for k in Car_res.keys():
            logger.info((k.ljust(10, ' ')) + str(Car_res[k]))
    logger.info('\n')
    return best_ap, best_log