...
def compute_centernet3d_loss(input, target):
    ...
    stats_dict['stat_loss'] = stats_dict['seg'] + stats_dict['offset2d'] + stats_dict['size2d'] + \
                             stats_dict['offset3d'] + stats_dict['depth'] + stats_dict['size3d'] + stats_dict['heading']

    total_loss = seg_loss + offset2d_loss + size2d_loss + offset3d_loss + depth_loss + size3d_loss + heading_loss
...
def compute_depth_loss(input, target):
    depth_input = extract_input_from_tensor(input['depth'], target['indices'], target['mask_2d'])
    depth_input, depth_log_variance = depth_input[:, 0:1], depth_input[:, 1:2]
    depth_input = 1. / (depth_input.sigmoid() + 1e-6) - 1.
    depth_target = extract_target_from_tensor(target['depth'], target['mask_2d'])
